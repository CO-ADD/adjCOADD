...
def main(prgArgs,djDir):
    ...
    from adjCOADD.dsummary.utils.summary_data import sum_structure_sc, sum_structure_dr
   ...
   # AssayData MIC -------------------------------------------------------------
    if prgArgs.table == 'Sum_Structure_List':
        ...
        if prgArgs.listfile:
            strDF = pd.read_csv(prgArgs.listfile)

        logger.info(f" [Sum Structure SC] TestWell: {prgArgs.listfile}  ")


        # for idx,row in tqdm(strDF.iterrows(), total = len(strDF), desc='[StructureLst SC]'):
        #     #print(cmpDict[cmps])
        #     _numbers,_outdict  = sum_structure_sc(row['new_structure_id'],upload=prgArgs.upload,overwrite=prgArgs.overwrite,appuser=prgArgs.appuser)
        ...
        for idx,row in tqdm(strDF.iterrows(), total = len(strDF), desc='[StructureLst DR]'):
            #print(cmpDict[cmps])
            _numbers,_outdict  = sum_structure_dr(row['new_structure_id'],upload=prgArgs.upload,overwrite=prgArgs.overwrite,appuser=prgArgs.appuser)
...
#    prgParser.add_argument("-d","--directory",default=None,required=False, dest="directory", action='store', help="Directory or Folder to parse")
    prgParser.add_argument("-l","--list",default=None,required=True, dest="listfile", action='store', help="Single CSV File")
