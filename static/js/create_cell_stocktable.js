function createCellStocktable(row, Batch_id_a, stock_create_id, stock_create_modal, permission) {
  // Define
        ...
        createdCell: function (cell, cellData, rowData, rowIndex, colIndex) {
          if (colIndex && colIndex === 7) {
            ...
            $('body').on("click", `#btn_save_${rowData.stock_id}_nleft`, function () {
              ...
              $.ajax({
                url: `/dcell/updateStock/${rowData.stock_id}/`,
                type: "POST",
                headers: { "X-CSRFToken": csrftoken },
  ...
  $(`#StockTable${Batch_id_a} tbody`).on("click", `#delete_stock`, function () {
    ...
    console.log(pk);
    my_modal.load(`/dcell/deleteStock/${pk}/`, function (response) {
  ...
  let url_stock = `/dcell/createStock/${Batch_id_a}/`
  ...
  $(`#StockTable${Batch_id_a} tbody`).on("click", `#update_stock`, function () {
    ...
    var pk = data["stock_id"].toString();
    console.log(pk);
    my_modal.load(`/dcell/updateStock/${pk}/`, function (response) {
      if (response == "Permission Not Granted") {
        alert("permission!");
      } else {
        my_modal.modal("show"); // Open Modal
